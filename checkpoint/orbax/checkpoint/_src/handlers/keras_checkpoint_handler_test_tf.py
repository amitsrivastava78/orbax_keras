...
"""Tests for KerasCheckpointHandler with TensorFlow backend."""
...
class KerasCheckpointHandlerTensorFlowTest(absltest.TestCase):
  ...
  def test_save_and_restore_tensorflow(self):
    """Test save/restore with TensorFlow backend."""
    self._test_save_and_restore_with_backend('tensorflow')
  ...
  def test_async_save_and_restore_tensorflow(self):
    """Test async save/restore with TensorFlow backend."""
    self._test_async_save_and_restore_with_backend('tensorflow')

  def test_async_checkpoint_during_training(self):
    """Test that async checkpointing doesn't block training and saves correct weights."""
    import asyncio
    import time
  ...
  def test_transforms_tf(self):
    """Test transformations with TensorFlow backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'tensorflow':
      self.skipTest(f"Test requires tensorflow backend, got {current_backend}")
  ...
  def test_transforms_with_key_renaming_tf(self):
  ...
  def test_custom_metadata_tf(self):
    """Test custom metadata support with TensorFlow backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'tensorflow':
      self.skipTest(f"Test requires tensorflow backend, got {current_backend}")
  ...
  def test_partial_restore_tf(self):
    """Test partial restore support with TensorFlow backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'tensorflow':
      self.skipTest(f"Test requires tensorflow backend, got {current_backend}")
