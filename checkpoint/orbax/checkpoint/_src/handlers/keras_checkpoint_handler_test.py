...
@unittest.skipIf(not KERAS_AVAILABLE, "Keras not available")
class KerasCompositeCheckpointHandlerTest(absltest.TestCase):
  ...
  def test_composite_save_and_restore_jax(self):
    """Test composite save/restore with JAX backend."""
    self._test_composite_save_and_restore_with_backend('jax')
  ...
  def test_composite_async_save_and_restore_jax(self):
    """Test async composite save/restore with JAX backend."""
    self._test_composite_async_save_and_restore_with_backend('jax')

  def test_composite_partial_restore_jax(self):
    """Test composite partial restore with JAX backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'jax':
      self.skipTest(f"Test requires jax backend, got {current_backend}")
  ...
  def test_composite_with_transforms_jax(self):
