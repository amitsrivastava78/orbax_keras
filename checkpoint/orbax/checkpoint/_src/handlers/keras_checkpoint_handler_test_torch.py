...
class KerasCheckpointHandlerPyTorchTest(absltest.TestCase):
  ...
  def test_custom_metadata_torch(self):
    """Test custom metadata support with PyTorch backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'torch':
      self.skipTest(f"Test requires torch backend, got {current_backend}")
  ...
  def test_enhanced_restore_args_torch(self):
    """Test enhanced restore args support with PyTorch backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'torch':
      self.skipTest(f"Test requires torch backend, got {current_backend}")
  ...
  def test_multihost_checkpointing_torch(self):
    """Test multi-host checkpointing support with PyTorch backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'torch':
      self.skipTest(f"Test requires torch backend, got {current_backend}")
...
@unittest.skipIf(not KERAS_AVAILABLE, "Keras not available")
class KerasCompositeCheckpointHandlerPyTorchTest(absltest.TestCase):
  ...
  def test_composite_save_and_restore_torch(self):
    """Test composite save/restore with PyTorch backend."""
    self._test_composite_save_and_restore_with_backend('torch')
  ...
  def test_composite_async_save_and_restore_torch(self):
    """Test async composite save/restore with PyTorch backend."""
    self._test_composite_async_save_and_restore_with_backend('torch')

  def test_composite_partial_restore_torch(self):
    """Test composite partial restore with PyTorch backend."""
    current_backend = keras.backend.backend()
    if current_backend != 'torch':
      self.skipTest(f"Test requires torch backend, got {current_backend}")
  ...
  def test_composite_with_transforms_torch(self):
